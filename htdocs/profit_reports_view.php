							...
							<img src="assets/img/Z.jpg" class="img-responsive  center-block" alt="" width="1000" height="600">
								...
								<table class="table table-hover">
									...
									<tbody>
										...
										WHERE  STR_TO_DATE(trans_date, '%d/%m/%Y') 
										...
										?>
											<tr class = "tbl_service">
												...
												<td class="col-md-1"><?php echo $obj[$i]["trans_amt"];?></td>
								 ...
								 <hr />
								  <div class="ttl-amts text-right">
									  <h4> <strong>Profit Amount :<?php 
													$counter = "select SUM(trans_amt) FROM tbl_trans_detail d INNER JOIN tbl_service_confirmation c ON d.serv_conf_id = c.serv_conf_id INNER JOIN tbl_service_request r ON r.request_id = c.request_id INNER JOIN tbl_service_detail s ON s.serv_detail_id = r.serv_detail_id  WHERE  STR_TO_DATE(trans_date, '%d/%m/%Y') between STR_TO_DATE('".$from."', '%d/%m/%Y') and STR_TO_DATE('".$to."', '%d/%m/%Y') ";
													echo $nRows = $pdo->query($counter)->fetchColumn();?> PHP</strong> </h4>
								 </div>
							</div>
						</div>
					</div>
					</div>
								<button class ="btn btn-info print" data-details = "Income Reports">Print</button>
					<!-- End  Hover Rows  -->
				</div>
